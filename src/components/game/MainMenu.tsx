...
import { Shirt, Map as MapIcon, Trophy, Star, BookOpen, Settings, Cloud, Swords, X, Users, Coins, ShoppingBag, Calendar, Crown } from 'lucide-react';
...
import { MAPS, SKINS, ACHIEVEMENTS } from '@/game/constants';
...
export function MainMenu({
  ...
  const setPendingSeed = useGameStore(state => state.setPendingSeed);
  ...
  const selectedMap = MAPS.find(m => m.id === biome);
  // Secret Unlock State
  ...
  const totalNotifications = friendRequests.length + pendingChallengesCount + rewardCount;
  ...
  // NEW: Calculate unclaimed achievements count
  ...
  const isMastered = useMemo(() => {
      const mapAchievements = ACHIEVEMENTS.filter(a => a.mapId === biome);
      if (mapAchievements.length === 0) return false;
      return mapAchievements.every(a => unlockedAchIds.has(a.id));
  }, [biome, unlockedAchIds]);
  ...
  const handleCancelChallenge = (e: React.MouseEvent) => {
      e.stopPropagation();
  ...
  const handleSecretTap = () => {
      ...
      if (secretTapsRef.current >= 7) {
          // FIX: Use 'glitch_duck' ID for the skin, not 'glitch' (which is the map ID)
      ...
      <div className="flex-1 w-full max-w-md flex flex-col items-center justify-center relative z-10 px-6 pt-12 pb-[calc(3rem+env(safe-area-inset-bottom))] min-h-0">
        ...
        <div className="w-full space-y-2 mb-3 shrink-0">
          ...
          >
            {pendingSeed ? (
                <div className="relative">
                    <Button
                        onClick={handleStart}
                        className="w-full h-20 text-2xl font-arcade bg-purple-500 hover:bg-purple-400 text-white border-4 border-black rounded-2xl shadow-hard-xl active:translate-y-[6px] active:shadow-none transition-all flex items-center justify-center gap-3 group relative overflow-hidden"
                    >
                        <Swords className="w-8 h-8 stroke-[3px]" />
                        <div className="flex flex-col items-center leading-none">
                            <span className="text-stroke-thick drop-shadow-md">ACCEPT</span>
                            <span className="text-xs font-black tracking-widest opacity-80">CHALLENGE</span>
                        </div>
                    </Button>
                    <button
                        onClick={handleCancelChallenge}
                        className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1.5 border-2 border-black shadow-sm hover:scale-110 transition-transform z-20"
                    >
                ...
                <>
                    ...
                    >
                        ...
                        <span className="text-xs sm:text-sm font-black tracking-widest opacity-90 uppercase bg-black/20 px-3 py-0.5 rounded-full">
                            {selectedMap?.name || 'LILY POND'}
                        </span>
                    </Button>
                    {isMastered && (
                        <div className="absolute -top-3 -right-3 z-20 pointer-events-none">
                             <div className="bg-yellow-400 text-yellow-900 p-2 rounded-full border-4 border-black shadow-sm animate-bounce">
                                <Crown className="w-6 h-6 fill-current stroke-[3px]" />
                             </div>
                        </div>
                    )}
                </>
          ...
          <motion.div whileTap={{ scale: 0.95 }} className="w-full">
             <Button
                onClick={() => { soundSynth.resumeContext(); soundSynth.playClick(); onDaily(); }}
                className="w-full h-12 text-base font-arcade bg-orange-500 hover:bg-orange-400 text-white border-4 border-black rounded-xl shadow-hard active:translate-y-[4px] active:shadow-none transition-all flex items-center justify-center gap-2 group relative overflow-hidden"
            >
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:animate-shimmer" />
                <Calendar className="w-5 h-5 stroke-[3px]" />
                <span className="text-stroke-1 drop-shadow-sm">DAILY CHALLENGE</span>
                <span className="text-[10px] bg-black/20 px-2 py-0.5 rounded-full ml-1 font-sans font-black">
                    {attemptsLeft}/3
                </span>
            </Button>
          </motion.div>
        </div>
        {/* Icon Grid - Compact 4 Columns */}
        <div className="grid grid-cols-4 gap-2 w-full px-1 pb-1 relative z-10 shrink-0">
          <MenuIconButton
            icon={Shirt}
          ...
          <MenuIconButton
            icon={MapIcon}
            onClick={() => { soundSynth.resumeContext(); soundSynth.playClick(); onBiome(); }}
            label="MAPS"
        ...
        <div
            className="w-full flex justify-center mt-4 pointer-events-auto cursor-pointer select-none pb-safe"
...
function MenuIconButton({ icon: Icon, onClick, label, color, badge, ariaLabel }: any) {
  const colorStyles: Record<string, string> = {
    ...
    // Updated to be more vibrant green as requested
  ...
  return (
    <div className="flex flex-col items-center gap-1 group">
      <motion.button
        ...
        className={cn(
          "w-full aspect-[4/3] flex items-center justify-center border-b-4 rounded-xl shadow-sm transition-all relative z-10",
          colorStyles[color]
        )}
      >
        <Icon className="w-5 h-5 stroke-[2.5px]" />
        {badge > 0 && (
            <div className="absolute -top-2 -right-2 bg-red-500 text-white text-[9px] font-bold w-4 h-4 flex items-center justify-center rounded-full border-2 border-white shadow-sm animate-bounce">
                {badge}
            </div>
        )}
      </motion.button>
      <span className="text-[9px] font-black uppercase text-white tracking-wider drop-shadow-[0_1px_1px_rgba(0,0,0,0.8)] relative z-10 group-hover:scale-105 transition-transform leading-none">
        {label}
      </span>
    </div>
  );
