...
import { UNLOCK_THRESHOLDS, MAPS, SKINS, ACHIEVEMENTS, GAME_CONSTANTS, CHALLENGE_TIERS, MAP_CHALLENGE_REWARDS } from '@/game/constants';
...
export const createProfileSlice: ProfileSliceCreator = (set, get) => ({
  ...
  updateStats: (stats) => {
    set((state) => {
      if (!state.profile) return {};
      const fullState = get();
      const gameMode = fullState.gameMode;
      ...
      let newCoins = (state.profile.coins || 0) + coinsEarned;
      ...
      let updatedCompletedChallengeIds = [...(state.profile.completedChallengeIds || [])];
      let challengeBonus = 0;
      const currentUnlocks = new Set(state.profile.unlockedSkinIds);
      if (gameMode === 'challenge') {
          const mapId = stats.biome;
          // Iterate tiers
          CHALLENGE_TIERS.forEach(tier => {
              const challengeId = `challenge-${mapId}-${tier.id}`; // e.g. challenge-pond-tier1
              if (!updatedCompletedChallengeIds.includes(challengeId) && score >= tier.time) {
                   updatedCompletedChallengeIds.push(challengeId);
                   if (tier.rewardCoins) {
                       newCoins += tier.rewardCoins;
                       challengeBonus += tier.rewardCoins;
                   }
                   if (tier.rewardType === 'skin') {
                       const skinId = MAP_CHALLENGE_REWARDS[mapId];
                       if (skinId && !currentUnlocks.has(skinId)) {
      ...
      const updatedProfile = {
        ...
        completedChallengeIds: updatedCompletedChallengeIds
      };
      // Submit Score
  ...
  logout: () => set({ profile: null, status: 'menu', friends: [], friendRequests: [], pbGhost: null }),
  ...
  fetchDailyLeaderboard: async () => {
      const { profile } = get();
      if (!profile) return;
      set({ isLoadingDailyLeaderboard: true, dailyLeaderboardError: null });
      try {
          const today = new Date().toISOString().split('T')[0];
          const dailyMap = getDailyMap(today);
          // Use 'daily_challenge' type to target the correct backend route
          const [lbRes, rankRes] = await Promise.all([
              api.getLeaderboard('any', 'daily_challenge'),
              api.getUserRank('any', profile.playerId, 'daily_challenge')
          ]);
          if (lbRes.success) {
              set({ dailyLeaderboard: lbRes.data || [], dailyMap });
          } else {
              set({ dailyLeaderboardError: 'Failed to load leaderboard' });
          }
          if (rankRes.success) {
              set({ userDailyRank: rankRes.data?.rank || null });
          }
      } catch (error) {
          set({ dailyLeaderboardError: 'Network error' });
      } finally {
          set({ isLoadingDailyLeaderboard: false });
      }
  },
  ...
  checkDailyReset: () => {
      ...
      if (!state.profile) return;
      const today = new Date().toISOString().split('T')[0];
      if (state.profile.lastDailyAttemptDate !== today) {
          const updated = { ...state.profile, dailyAttempts: 0, lastDailyAttemptDate: today };
          set({ profile: updated });
          api.syncUser(updated).catch(console.error);
      }
  },
  startDailyChallenge: () => {
      ...
      if (!state.profile) return;
      const today = new Date().toISOString().split('T')[0];
      const newAttempts = (state.profile.dailyAttempts || 0) + 1;
      const updated = { ...state.profile, dailyAttempts: newAttempts, lastDailyAttemptDate: today };
      set({ profile: updated });
      api.syncUser(updated).catch(console.error);
      // Game Setup
      const dailyMap = getDailyMap(today);
      ...
      set({
          ...
          status: 'playing',
          biome: dailyMap.id,
          previousBiome: currentBiome, // Save current biome to restore later
